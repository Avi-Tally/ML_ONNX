...
print(f"Total Payables fetched: {len(bills)}")
...
for b in bills:
    ...
    if start_dt <= p_due <= ref_dt:
        found.append((b, p_due, age))

print(f"\nFound {len(found)} payables in the due date range [{start_dt.strftime('%Y-%m-%d')}, {ref_dt.strftime('%Y-%m-%d')}]:")
for b, p_due, age in found:
    print(f"  Party: {b.get('party'):<30} | Bill: {b.get('name'):<20} | Date: {b.get('date')} | Due: {b.get('due_date')} (Computed: {p_due.strftime('%Y-%m-%d')}) | Age: {age}d | Amt: {b.get('amount')}")
